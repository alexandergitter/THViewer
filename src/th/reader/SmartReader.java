...
public class SmartReader extends DefaultHandler {
    ...
    public JComponent open() {
        ...
        if(as.getValue("type").equals("chunks")) {
            ...
            File paletteFile = openFromString(as.getValue("palette"));
            File chunksFile = openFromString(as.getValue("chunks"));
            File spriteElementFile = openFromString(as.getValue("spriteelement"));
            File listFile = openFromString(as.getValue("list"));
            return new FramePanel(paletteFile, chunksFile, tabFile, spriteElementFile, f, listFile);
            ...
            File listFile = openFromString(as.getValue("list"));
            return new AnimationPanel(paletteFile, chunksFile, tabFile, spriteElementFile, framesFile, listFile, f);
            ...
            return new SavegamePanel(frameFile, frameListFile, frameElementFile,
                    frameTabFile, frameChunksFile, framePaletteFile, saveFile,
                    blockTabFile, blockChunksFile, blockPaletteFile);
        ...
        try {
            ...
            if(c == 'R') {
                ...
                if(c == 'N') {
                    ...
                    if(c == 'C') {
                        Process dernc = Runtime.getRuntime().exec("dernc \"" + file.getAbsoluteFile() + "\"");
                        dernc.waitFor();
                    }
                }
            }
